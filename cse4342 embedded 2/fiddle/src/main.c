...
int main(){
	...
	{ // button exti
		...
		SYSCFG->EXTICR[3] |= 0x0020;// select port for pin
		
		EXTI->IMR |= 0x2000; // enable interrupt
		EXTI->FTSR |= 0x2000; // enable falling edge trigger
...
void EXTI15_10_IRQHandler(void) { 
		...
		NVIC_DisableIRQ(EXTI15_10_IRQn); // disable btn
...
void TIM2_IRQHandler(void) { // debounce
	TIM2->SR &= ~TIM_SR_UIF;
	...
	NVIC_EnableIRQ(EXTI15_10_IRQn); // reenable btn
}

void TIM3_IRQHandler(void) { // window
	TIM3->SR &= ~TIM_SR_UIF;
