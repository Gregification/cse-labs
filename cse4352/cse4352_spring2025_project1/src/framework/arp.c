...
void sendArpRequest(etherHeader *ether, uint8_t ipFrom[], uint8_t ipTo[])
{
    ...
    uint8_t lowest = 0;
    for(i = 1; i < ARP_BUFFER_SIZE; i++){
        if(arpLot[i].ipv4.raw == 0){
            lowest = i;
            break;
        }
       if(!arpLot[i].pending && arpLot[lowest].priority > arpLot[i].priority)
           lowest = i;
    }
    arpLot[lowest].ipv4.raw = *(uint32_t *)ipTo;
    arpLot[lowest].pending = true;
}
