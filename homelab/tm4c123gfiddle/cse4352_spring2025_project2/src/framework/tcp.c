    ...
    for(i = 0; i < TCP_PENQUE_MAX_MSG_COUNT; i++){
        ...
        if(pm->socket && pm->datasize) { // message exist
            if(pm->socket->state == TCP_ESTABLISHED){
                // send message
                sendTcpMessage(ether, pm->socket, PSH | ACK, pm->data, pm->datasize);
...
void processTcpResponse(socketInfo * s, etherHeader * e)
{
    ...
    s->sock->acknowledgementNumber = ntohl(tcp->sequenceNumber);
...
//            }
        ...
        case TCP_LISTEN:
            initSockInfoState(si); // reset timeout info, to prevent timeout
            break;
